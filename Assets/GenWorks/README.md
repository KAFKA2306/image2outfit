...
- `Legacy/` — 旧配置から移行した監査対象。新規制作や自動リリースには使用しない

`Assets/GenWorks/Legacy/Published/` には、旧リポジトリ直下の `Published/` に保存されていた履歴スナップショットを、内容と既存blobを維持したまま配置します。Unity ProjectウィンドウからFBX、Prefab、テクスチャ、プレビュー、監査証拠を直接確認できますが、`Products/` の製品カタログには登録せず、販売可能判定にも自動昇格させません。

...
DESIGN_REVISION = "v1-dmap-pattern-coordinate-hooded-bodysuit"
...
def main() -> int:
    ...
    texture_paths, fabric, trim, buttons = materials.create_materials(product_root / "Textures")
    garments = geometry.create_outfit(body, armature, fabric, trim, buttons)
    geometry.clean_meshes(garments)
    ...
    previews = {name: base.repo_path(value) for name, value in job["previewPaths"].items()}
    ...
    report = {
        ...
        "views": {
            name: {"path": str(path.relative_to(ROOT)).replace("\\", "/"), "sha256": base.sha256(path), "width": Image.open(path).width, "height": Image.open(path).height}
    ...
    (report_dir / "product-build-report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    ...
    manifest = {
        ...
        "handoff": {
            ...
            "lastAttempt": {"result": "HOSTED_MODELED" if passed else "REJECTED", "visualRevision": DESIGN_REVISION},
            "blockers": ["Inspect actual five-view and pose-review images", "Import/save/reload both Prefabs in pinned Unity", "Pass Modular Avatar/NDMF and VRChat Build & Test", "Complete human visual, pose and runtime reviews"],
        },
        "technicalGates": {
            "standardTargetResolved": "PASS",
            "blender": "PASS" if passed else "FAIL",
            "fbx": "PASS" if fbx_path.is_file() else "FAIL",
            "fiveViewRender": "PASS" if all(path.is_file() for path in previews.values()) else "FAIL",
            "poseRender": "PENDING",
        ...
        "outputs": {
            "blend": job["blendPath"], "fbx": job["fbxAssetPath"], "prefab": job["prefabAssetPath"], "integratedPrefab": job["integratedPrefabAssetPath"],
    ...
    base.repo_path(job["productManifestPath"]).write_text(json.dumps(manifest, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    tracked_files = sorted(path for path in product_root.rglob("*") if path.is_file() and path.name != "SOURCE_HASHES.txt")
    (product_root / "SOURCE_HASHES.txt").write_text("\n".join(f"{base.sha256(path)}  {path.relative_to(product_root).as_posix()}" for path in tracked_files) + "\n", encoding="utf-8")

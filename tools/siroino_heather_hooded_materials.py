...
def make_heather_maps(directory: Path) -> dict[str, Path]:
    ...
    for y in range(size):
        for x in range(size):
            ...
            value = max(112, min(168, int(142 + 6 * yarn + 4 * cross + 3 * fleck)))
            albedo.append((value - 3, value, min(255, value + 7)))
            normal.append((int(128 + 10 * yarn), int(128 + 7 * cross), 253))
            rough = int(188 + 12 * abs(fleck) + 5 * (1.0 - abs(yarn)))
...
def create_materials(texture_dir: Path) -> tuple[dict[str, Path], bpy.types.Material, bpy.types.Material, bpy.types.Material]:
    ...
    if "Coat Weight" in shader.inputs:
        shader.inputs["Coat Weight"].default_value = 0.02
    trim = base.plain_material(
        "MAT_Heather_Rib_Trim", (0.30, 0.34, 0.41, 1.0), roughness=0.76
    )
    buttons = base.plain_material(
        "MAT_Pearl_Grey_Buttons", (0.50, 0.54, 0.61, 1.0), roughness=0.34, metallic=0.05
    )

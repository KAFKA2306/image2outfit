#!/usr/bin/env python3
"""Build Cyber Kawaii for the official unmodified Siroino PC body."""
...
def apply_standard_profile(body: bpy.types.Object, requested=None) -> dict[str, object]:
    """Reset every body shape key and retain the neutral official PC mesh."""
...
def near_segment(point: Vector, start: Vector, end: Vector, t0: float, t1: float, radius: float) -> bool:
    ...
    if t < t0 or t > t1:
        return False
    return (point - (start + direction * t)).length <= radius
...
def stable_shell(
    ...
    *,
    ...
    thickness: float,
    group: str = "Hips",
    fold_strength: float = 0.02,
) -> bpy.types.Object:
    """Create a closed shell without Solidify/Bevel zero-area triangles."""
    segments = max(48, pleats * 4)
    outer_count = len(rings) * segments
    vertices: list[tuple[float, float, float]] = []
    faces: list[tuple[int, int, int, int]] = []

    def coordinate(z: float, rx: float, ry: float, index: int, inset: float):
    ...
    for z, rx, ry in rings:
        vertices.extend(coordinate(z, rx, ry, index, thickness) for index in range(segments))
    ...
    z_top = rings[0][0]
    z_span = max(1e-6, abs(rings[0][0] - rings[-1][0]))
    for polygon in mesh.polygons:
        for loop_index in polygon.loop_indices:
            vertex_index = mesh.loops[loop_index].vertex_index
            local_index = vertex_index % segments
            u = local_index / segments
...
def box_ribbon(name, center, size, material, armature, group="Chest") -> bpy.types.Object:
    cx, cy, cz = center
    sx, sy, sz = size
    vertices = [
...
def create_outfit(body, armature, materials) -> list[bpy.types.Object]:
    ...
    garments.extend(
        [
            base.extract_surface(body, armature, "White_Cropped_Blouse_Front", lambda c: 0.835 <= c.z <= 1.018 and c.y < 0.005 and abs(c.x) <= 0.145, white, 0.0080),
            base.extract_surface(body, armature, "White_Cropped_Blouse_Back", lambda c: 0.842 <= c.z <= 0.990 and c.y >= -0.006 and abs(c.x) <= 0.143, white, 0.0078),
            base.extract_surface(body, armature, "White_Waist_Base", lambda c: 0.708 <= c.z <= 0.795 and abs(c.x) <= 0.155, white, 0.0070),
        ]
    )
    garments.extend(
        [
            stable_shell("Black_Pink_Plaid_Pleated_Skirt", armature, plaid, ((0.790, 0.150, 0.110), (0.706, 0.174, 0.128), (0.625, 0.198, 0.147)), pleats=16, thickness=0.0015),
            stable_shell("White_Ruffle_Underskirt", armature, white, ((0.650, 0.184, 0.137), (0.595, 0.207, 0.154)), pleats=20, thickness=0.0013, fold_strength=0.016),
            stable_shell("Black_Skirt_Waistband", armature, black, ((0.795, 0.152, 0.112), (0.777, 0.155, 0.114)), pleats=16, thickness=0.0014, fold_strength=0.008),
            stable_shell("Pink_Underskirt_Hem", armature, pink, ((0.605, 0.202, 0.151), (0.590, 0.208, 0.155)), pleats=20, thickness=0.0012, fold_strength=0.014),
            base.extract_surface(body, armature, "White_Thigh_High_Stockings", lambda c: 0.090 <= c.z <= 0.490 and abs(c.x) >= 0.016, white, 0.0062),
        ]
    )
    ...
    for side_name in ("L", "R"):
        ...
        garments.append(
            base.extract_surface(
                ...
                lambda c, a=upper_start, b=upper_end: near_segment(c, a, b, 0.02, 0.32, 0.068),
                white,
                0.0100,
            )
        )
        garments.append(
            base.extract_surface(
                ...
                lambda c, a=lower_start, b=lower_end: near_segment(c, a, b, 0.10, 0.82, 0.050),
                white,
                0.0075,
            )
        )

    bow_y = base.body_front_y(body, 0.0, 0.958) - 0.016
    garments.extend(
        [
            box_ribbon("Pink_Collar_Bow_L", (-0.014, bow_y, 0.958), (0.014, 0.0032, 0.008), pink, armature),
            box_ribbon("Pink_Collar_Bow_R", (0.014, bow_y, 0.958), (0.014, 0.0032, 0.008), pink, armature),
            box_ribbon("Pink_Collar_Bow_Knot", (0.0, bow_y - 0.001, 0.958), (0.006, 0.0040, 0.006), pink, armature),
        ]
    )
    return garments
...
def robust_clean_meshes(objects: list[bpy.types.Object]) -> None:
    for obj in objects:
        ...
        for _ in range(3):
            ...
            doomed = [cleanup.faces[index] for index in sorted(bad) if index < len(cleanup.faces)]
            if doomed:
...
def refresh_hashes(product_root: Path) -> None:
    files = sorted(path for path in product_root.rglob("*") if path.is_file() and path.name != "SOURCE_HASHES.txt")
    (product_root / "SOURCE_HASHES.txt").write_text(
        "\n".join(f"{base.sha256(path)}  {path.relative_to(product_root).as_posix()}" for path in files) + "\n",
        encoding="utf-8",
    )
...
def rewrite_handoff(job: dict, return_code: int) -> None:
    ...
    report_path = repo_path(job["artifactDir"]) / "product-build-report.json"
    ...
    report["notes"] = [
        "The official neutral SiroinoSotai PC FBX is the target source.",
        "No _Large shape keys are required or applied.",
        "The skirt is a closed shell weighted to Hips; sleeves and stockings inherit official body weights.",

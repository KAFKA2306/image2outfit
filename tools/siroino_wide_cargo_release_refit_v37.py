#!/usr/bin/env python3
"""Converged-crotch geometry for Siroino wide cargo trousers.

The upper-leg cross-sections transition from separated tubes into overlapping
D-shaped sections directly above the fork.  From z=.58 upward, both sections
share the front and rear centre line.  This closes the skin slit and keeps the
recessed waist shell fully hidden behind the trousers.
"""
...
def build_geometry(segments: int = 48) -> v32.MeshBuilder:
    ...
    # Recessed seat/waist shell. Its complete lower edge is behind the merged
    # upper-leg sections, so it cannot appear as an independent centre panel.
    pelvis_specs = [
    ...
    # The fork transition happens over 40 mm. Above z=.58, centre_x is zero:
    # front and rear points coincide at x=0 while the inner side overlaps by
    # 16 mm. This creates a visible trouser seam rather than exposed skin.
    leg_specs = [
...
def audit() -> dict[str, object]:
    ...
    seat = v32.band(garment, 0.620, 0.800)
...
def record(report: dict[str, object]) -> None:
    ...
    manifest["designRevision"] = "v37-converged-crotch-final"
...
if __name__ == "__main__":
    ...
    if not result["passed"]:
        raise RuntimeError(f"v37 final garment audit failed: {result}")
    raise SystemExit(0)

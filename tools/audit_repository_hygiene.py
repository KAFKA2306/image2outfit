...
DIRECT_PRODUCT_CONFIG_PATTERNS = (
    "*-job.json",
    "*-license.json",
    "*-approval.json",
)
...
def audit(root: Path = ROOT) -> dict[str, Any]:
    ...
    if config_root.is_dir():
        ...
        for pattern in DIRECT_PRODUCT_CONFIG_PATTERNS:
            for path in sorted(config_root.glob(pattern)):
                add(
                    findings,
                    "direct-product-config",
                    path,
                    root,
                    "product configuration is mixed into the global config root",
                )

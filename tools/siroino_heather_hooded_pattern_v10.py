...
def _panel_predicate(front: bool) -> Callable[[Vector], bool]:
    def selected(point: Vector) -> bool:
        # A 50 mm overlap at the side seam prevents the front/back split from
        # revealing the avatar while preserving separate authored panels.
        ...
        # Curved Henley neckline: lower at center front, higher over shoulders.
        ...
        # A broader, continuously connected lower panel replaces the v10
        # 20 mm crotch strip and removes the sawtooth high-cut fins.
        if 0.650 <= z < 0.825:
            t = (z - 0.650) / 0.175
...
def _hood_half(
    ...
    """Create a hood-down drape whose neckline and side edge touch the garment."""
    profiles = [
        (1.030, 0.086, 0.010),
        (1.006, 0.102, 0.025),
        (0.976, 0.118, 0.047),

#!/usr/bin/env python3
"""Rear-safe, spike-free Siroino Wide Cargo build."""
...
def create_outfit(body, armature, fabric, strap, metal):
    del metal
    pants = build.c.extract_surface(body, armature, "Cargo_Continuous_Pants", v23.pants_surface, fabric, 0.011)
...
def audit() -> dict[str, object]:
    ...
    inner = [vertex.co for vertex in pants.data.vertices if 0.440 <= vertex.co.z <= 0.610 and abs(vertex.co.x) <= 0.045]
    rear_bridge = [point for point in inner if point.y >= 0.018 and 0.470 <= point.z <= 0.610]
    ...
    required = [
        "singleShellOnly", "finiteCoordinatesPassed", "stretchedSourceFacesRemoved",
        "spikeGuardPassed", "topologyPassed", "uvPassed", "materialSeparationPassed",
        "shapeKeyIsolationPassed", "footAndFloorClearancePassed", "controlledVolumePassed",
        "profileContinuityPassed", "rearSeatClearancePassed", "innerThighCoveragePassed",
...
def record(report: dict[str, object]) -> None:
    ...
    manifest["designRevision"] = "v28-rear-safe-stretched-face-free"
...
if __name__ == "__main__":
    ...
    if not result["passed"]:
        raise RuntimeError(f"v28 rear/topology audit failed: {result}")
    raise SystemExit(0)

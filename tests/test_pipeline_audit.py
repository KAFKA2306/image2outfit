...
from image2outfit.audit import validate_stage_records, verify_audit_bundle as _verify_audit_bundle, write_audit_bundle as _write_audit_bundle
...
_PROFILE = json.loads((ROOT / "config/pipeline-profiles/garment-reconstruction-v1.json").read_text(encoding="utf-8"))
_, RECORD_SCHEMA, _, MANIFEST_SCHEMA = load_audit_schemas(ROOT, _PROFILE["auditContract"])
...
class PipelineAuditTests(unittest.TestCase):
    ...
    def _registry(*, failing_stage: str | None = None) -> ToolRegistry:
        ...
        for stage in PIPELINE_STAGES:
            def handler(state, stage_name=stage.value):
                ...
                return {"mode": "planned", "stage": stage_name, "productId": state["product_id"]}
            registry.register(stage, handler, ToolDescriptor(tool_name=f"test-{stage.value}", purpose=f"test {stage.value}", output_contract=f"pipeline-output/{stage.value}.json"))
        return registry

    @classmethod
    def _result(cls, *, product_id: str, run_id: str):
        return run_pipeline(new_pipeline_state(product_id=product_id, target_avatar="SiroinoSotai_PC", source_reference=f"private-reference://sha256/{product_id}", run_id=run_id), cls._registry())
    ...
    def test_every_stage_is_recorded_and_persisted(self) -> None:
        ...
        validate_stage_records(records, expected_run_id="audit-run-001", expected_product_id="audit-garment", canonical_stages=CANONICAL_STAGES, record_schema=RECORD_SCHEMA)
        self.assertEqual([record["stage"] for record in records], CANONICAL_STAGES)
        with TemporaryDirectory() as temporary:
            audit_root = Path(temporary) / "audit"
            bundle = write_audit_bundle(result, audit_root=audit_root, canonical_stages=CANONICAL_STAGES)
    ...
    def test_failed_stage_has_an_auditable_output(self) -> None:
        ...
        result = run_pipeline(new_pipeline_state(product_id="audit-failure", target_avatar="SiroinoSotai_PC", source_reference="private-reference://sha256/failure", run_id="audit-run-failure"), self._registry(failing_stage=failing.value))
        ...
        validate_stage_records(result["stage_records"], canonical_stages=CANONICAL_STAGES, record_schema=RECORD_SCHEMA)

    def test_modified_stage_file_is_rejected(self) -> None:
        result = self._result(product_id="audit-tamper", run_id="audit-run-tamper")
        with TemporaryDirectory() as temporary:
            bundle = write_audit_bundle(result, audit_root=Path(temporary) / "audit", canonical_stages=CANONICAL_STAGES)
            run_root = Path(bundle["root"])
    ...
    def test_existing_run_cannot_be_overwritten(self) -> None:
        result = self._result(product_id="audit-immutable", run_id="audit-run-immutable")
        with TemporaryDirectory() as temporary:
            audit_root = Path(temporary) / "audit"
            write_audit_bundle(result, audit_root=audit_root, canonical_stages=CANONICAL_STAGES)

...
class NocturneAngelSetContractTest(unittest.TestCase):
    def test_job_uses_stable_canonical_entrypoints(self) -> None:
        ...
        self.assertEqual(job["renderLoopRevision"], "nocturne-angel-loop-v7-scope-fix")
    ...
    def test_v7_closes_front_and_rebalances_skirt_and_wings(self) -> None:
        source = generator_source()
        for required in (
            "sewn_bodice_shell",
            ...
            "ellipsoid_between",
            "four laterally fanned volumetric feathers per side",
    ...
    def test_v7_preserves_supported_modules_and_v6_rejection_history(self) -> None:
        source = generator_source()
        for object_name in (
            "Nocturne_Sewn_Bodice",
            "Nocturne_Bodice_Front_Inset",
        ...
        self.assertTrue((PRODUCT / "Tests" / "visual-review-v6.json").is_file())
        self.assertIn('"v6-welded-projected-pleats"', source)
        job = json.loads((CONFIG / "job.json").read_text(encoding="utf-8"))
        self.assertIn(
            "Assets/GenWorks/siroino-nocturne-angel-set/Tests/visual-review-v6.json",
            job["deliveryAssets"],
        )

...
class NocturneAngelSetContractTest(unittest.TestCase):
    def test_job_uses_stable_canonical_entrypoints(self) -> None:
        ...
        self.assertEqual(job["renderLoopRevision"], "nocturne-angel-loop-v4")
    ...
    def test_construction_is_explicit_loose_layered(self) -> None:
        ...
        for required in ("skirt", "sailor collar", "wing", "beret", "leg warmers"):
    ...
    def test_v4_uses_clearance_and_articulated_modules(self) -> None:
        source = generator_source()
        for required in (
            ...
            self.assertIn(required, source)
        self.assertIn("maximum_leg_weight * lower_fraction**1.35", source)
        self.assertIn("clearanceAdjustments", source)

    def test_v4_preserves_named_modular_objects_and_rejection_history(self) -> None:
        source = generator_source()
        for object_name in (
        ...
        self.assertTrue((PRODUCT / "Tests" / "visual-review-v3.json").is_file())
        self.assertIn('"v3-sewn-v-neck-stable-modules"', source)

...
class ReleaseGateTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "Assets" / "_Local" / "Generated" / "test-product").mkdir(
            parents=True
        )
        (self.root / "Assets" / "_Local" / "Jobs" / "test-product").mkdir(
            parents=True
        )
        (self.root / "Assets" / "_Local" / "Evidence" / "test-product").mkdir(
            parents=True
        )
        ...
        self.job_path = (
            self.root / "Assets" / "_Local" / "Jobs" / "test-product" / "job.json"
        )
    ...
    def test_blocked_adapter_is_rejected_with_passing_evidence(self) -> None:
        ...
        audit = gate.read(self.root / self.job["artifactDir"] / "audit.json")
        self.assertIn(
            "adapter blocked from release: blocked-adapter-v1",
            audit["errors"],
        )
    ...
    def test_release_requires_unchanged_hash_bound_evidence(self) -> None:
        ...
        self.assertTrue(
            (self.root / self.job["releaseDir"] / "test-product.zip").is_file()
        )

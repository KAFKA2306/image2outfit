...
class ResearchBaselineTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.path = (
            self.root / "Assets/GenWorks/Shared/Research/2026-garment-methods.json"
        )
    ...
    def test_candidate_orchestrator_resolves_relative_baseline_from_active_root(
        self,
    ) -> None:
    ...
    def test_unlicensed_code_reuse_is_rejected(self) -> None:
        ...
        self.assertTrue(
            any(
                "REUSE requires" in error or "verified for REUSE" in error
    ...
    def test_unknown_requirement_source_is_rejected(self) -> None:
        ...
        self.assertFalse(result["passed"])
        self.assertTrue(
            any("unknown publications" in error for error in result["errors"])
        )
